...
#[derive(Clone)]
struct ChordTemplate { name: String, root: usize, tones: [f32; 12] }

#[derive(Clone, Serialize)]
struct ChordCandidate { chord: String, confidence: f32 }
...
fn chord_templates() -> Vec<ChordTemplate> {
    ...
    for root in 0..12 {
        for (suffix, intervals) in qualities {
            ...
            for interval in intervals { tones[(root + interval) % 12] = 1.0; }
            output.push(ChordTemplate { name: format!("{}{}", NOTE_NAMES[root], suffix), root, tones });
        }
    }
    output
}
...
fn normalize(values: &mut [f32; 12]) {
    ...
    if sum > 1e-9 { for value in values { *value /= sum; } }
}
...
fn frame_features(frame: &[f32], sample_rate: u32) -> ([f32; 12], [f32; 12], f32) {
    ...
    for (bin, value) in buffer.iter().take(size / 2).enumerate().skip(1) {
        ...
        if !(45.0..=5000.0).contains(&frequency) { continue; }
        ...
        if frequency <= 260.0 { bass[pitch_class as usize] += magnitude * (260.0 / frequency.max(45.0)).sqrt(); }
    }
    normalize(&mut chroma); normalize(&mut bass);
...
fn emission_score(chroma: &[f32; 12], bass: &[f32; 12], template: &ChordTemplate, energy: f32) -> f32 {
    if energy < 0.002 { return -2.0; }
    let active = template.tones.iter().filter(|v| **v > 0.0).count() as f32;
    ...
    inside * 4.0 - outside * 1.5 + bass[template.root] * 0.75 + bass[(template.root + 7) % 12] * 0.15
}

fn transition_score(previous: &ChordTemplate, current: &ChordTemplate) -> f32 {
    if previous.name == current.name { return 0.45; }
    let root_distance = (12 + current.root as i32 - previous.root as i32) % 12;
    let common = previous.tones.iter().zip(current.tones).filter(|(a,b)| **a > 0.0 && *b > 0.0).count() as f32;
    let motion = match root_distance { 5 | 7 => 0.12, 2 | 10 => 0.06, _ => 0.0 };
    common * 0.025 + motion - 0.18
}

fn estimate_tempo(samples: &[f32], sample_rate: u32) -> (f32, Vec<f32>) {
    let hop = (sample_rate as usize / 100).max(1);
    let envelope: Vec<f32> = samples.chunks(hop).map(|c| c.iter().map(|v| v.abs()).sum::<f32>() / c.len().max(1) as f32).collect();
    let onset: Vec<f32> = envelope.windows(2).map(|w| (w[1]-w[0]).max(0.0)).collect();
    if onset.len() < 200 { return (120.0, onset); }
...
fn estimate_key(chroma: &[f32; 12]) -> String {
    ...
    for root in 0..12 { for (is_major, profile) in [(true,major),(false,minor)] {
        let score: f32 = (0..12).map(|i| chroma[(i+root)%12] * profile[i]).sum();
        if score > best.2 { best = (root,is_major,score); }
    }}
...
pub fn analyze_audio(samples: &[f32], sample_rate: u32) -> String {
    ...
    for segment in samples.get(offset_samples..).unwrap_or(&[]).chunks(segment_samples) {
        ...
        for i in 0..12 { global_chroma[i] += feature.0[i]; }
    ...
    let mut emissions = vec![vec![0.0f32; states]; frames];
    for (t,(chroma,bass,energy)) in features.iter().enumerate() {
        for (s,template) in templates.iter().enumerate() { emissions[t][s] = emission_score(chroma,bass,template,*energy); }
    }
    ...
    for t in 1..frames {
        ...
        for current in 0..states {
            ...
            for prev in 0..states {
                ...
                if score > best.1 { best = (prev,score); }
            }
            next[current] = best.1 + emissions[t][current]; back[t][current] = best.0;
        }
        previous = next;
    }
    let mut path = vec![0usize; frames];
    if frames > 0 {
        let mut state = previous.iter().enumerate().max_by(|a,b| a.1.total_cmp(b.1)).map(|x| x.0).unwrap_or(0);
        path[frames-1] = state;
